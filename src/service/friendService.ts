...
export async function createFriendPr({
  ...
  const branch = `add-friend-${name.replace(/[^a-zA-Z0-9]/g, '')}-${link.replace(/[^a-zA-Z0-9]/g, '')}-${timestamp}`;
...
// 生成友链条目的 YAML 格式字符串
export function generateFriendYamlEntry(friend: Omit<FriendApply, 'email' | 'state'>): string {
  return `\n    - name: ${friend.name}\n      link: ${friend.link}\n      avatar: ${friend.avatarLink}\n      descr: ${friend.descr}`;
}

// 计算文件内容的差异
export function calculateFileDiff(oldContent: string, newContent: string): string {
...
// 获取原始友链信息
export async function getOriginalFriendInfo(originalLink: string): Promise<string | null> {
  const octokit = new Octokit({ auth: GITHUB_TOKEN });
  const owner = GITHUB_OWNER;
  const repo = GITHUB_REPO;
  const filePath = LINK_FILE_PATH;
        ...
        for (const friendLink of category.link_list) {
          if (friendLink.link) {
            ...
            if (normalizedFriendLink === normalizedOriginalLink) {
              return '- ' + yaml.dump(friendLink).replace(/\n/g, '\n  ').trim() + '\n';
            }
          }
        }
      }
...
export async function updateFriendPr(
  ...
  try {
    ...
    const formattedNewContent = newContent.replace(/^(\s*)\n-/gm, '$1\n  -').trim();
    const newEntry = generateFriendYamlEntry(newFriend);

    const timestamp = Date.now();
    const branch = `update-friend-${newFriend.name.replace(/[^a-zA-Z0-9]/g, '')}-${timestamp}`;
